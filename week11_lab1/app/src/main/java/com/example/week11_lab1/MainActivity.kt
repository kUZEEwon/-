...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        writeBtn.setOnClickListener {
            ...
            if(EditName.length() == 0 ) Name = "null"
            else Name = EditName.text.toString()

            if(EditEmail.length() == 0 ) Email = "null"
            else Email = EditEmail.text.toString()

            if(EditAge.length() == 0 ) Age = 0
            else Age = EditAge.text.toString().toInt()
    ...
    fun readFirebase(){
        //Collection 객체 연결
        //get()으로 데이터 가져오기
        //addOnSuccessListen, addOnFailListener 기능 구현
